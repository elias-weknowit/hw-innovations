...
async function handlePUT(req: NextApiRequest, res: NextApiResponse, cookie: DecodedIdToken) {
    ...
    if(!snapshot){
        ...
        if(snapshot.docs.length === 0){
            ...
            const docRef = doc(db, 'advertisements', adId).withConverter(timestampConverter);
            await updateDoc(docRef, updateObject).then(result => res.status(200).json(result)).catch( err => internalError(res, err));
        }
    }
}
